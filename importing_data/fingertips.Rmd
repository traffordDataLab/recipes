...
2. Run the `select_indicators()` function from the <a href="https://cran.r-project.org/web/packages/fingertipsR/index.html" target="_blank">fingertipsR</a> package. Enter your search terms into the popup browser window and find the corresponding **IndicatorID** in the table. We are interested in <a href="https://fingertips.phe.org.uk/search/91414" target="_blank">alcohol related admissions</a> so we need **IndicatorID** = 91414.
...
4. Next we need to choose our area type. To obtain a list of area types we'll use the `area_types()` function. We want values of the indicator for a local authority so need **AreaTypeID** = 101.
```{r, eval=FALSE}
area_types() %>%
